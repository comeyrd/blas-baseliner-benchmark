...
namespace GpuBlas {
  template <>
  void CudaGemm<float>::run_case(std::shared_ptr<typename backend::stream_t> stream) {
  ...
  template <>
  void CudaGemm<double>::run_case(std::shared_ptr<typename backend::stream_t> stream) {
    CHECK_CUBLAS(cublasDgemm(this->handle, cublasOperation_t::CUBLAS_OP_N, cublasOperation_t::CUBLAS_OP_N, this->m_m,
                             this->m_n, this->m_k, &this->m_alpha, this->m_d_A, this->m_m, this->m_d_B, this->m_k,
                             &this->m_beta, this->m_d_C, this->m_m));
  };
  template <>
  void CudaGemm<cuComplex>::run_case(std::shared_ptr<typename backend::stream_t> stream) {
    CHECK_CUBLAS(cublasCgemm(this->handle, cublasOperation_t::CUBLAS_OP_N, cublasOperation_t::CUBLAS_OP_N, this->m_m,
                             this->m_n, this->m_k, &this->m_alpha, this->m_d_A, this->m_m, this->m_d_B, this->m_k,
                             &this->m_beta, this->m_d_C, this->m_m));
  };
  template <>
  void CudaGemm<cuDoubleComplex>::run_case(std::shared_ptr<typename backend::stream_t> stream) {
  ...
  namespace {
    ...
    BASELINER_REGISTER_CASE(sgemm);
    BASELINER_REGISTER_CASE(dgemm);
    BASELINER_REGISTER_CASE(cgemm);
    BASELINER_REGISTER_CASE(zgemm);
  } // namespace
