...
namespace GpuBlas {
  ...
  namespace {
    ...
    using hgemm_h16_acc = CublasGemmEx<Shapes::TypeConfig<__half, __half, __half>, int>;
    using hgemm_h16_acc_64 = CublasGemmEx<Shapes::TypeConfig<__half, __half, __half>, int64_t>;
    BASELINER_REGISTER_WORKLOAD(hgemm_h16_acc);
    BASELINER_REGISTER_WORKLOAD(hgemm_h16_acc_64);

    using hgemm_h16_acc_pedantic = CublasGemmEx<Shapes::TypeConfig<__half, __half, __half, PedanticMath>, int>;
    BASELINER_REGISTER_WORKLOAD(hgemm_h16_acc_pedantic);

    // --- BFloat16 (Compute 32F) ---
    using bfgemm_f32_acc = CublasGemmEx<Shapes::TypeConfig<__nv_bfloat16, float, __nv_bfloat16>, int>;
    using bfgemm_f32_acc_64 = CublasGemmEx<Shapes::TypeConfig<__nv_bfloat16, float, __nv_bfloat16>, int64_t>;
    ...
    using bfgemm_f32_out_f32_acc = CublasGemmEx<Shapes::TypeConfig<__nv_bfloat16, float, float>, int>;
    ...
    using igemm_f32_out_f32_acc = CublasGemmEx<Shapes::TypeConfig<int8_t, float, float>, int>;
    BASELINER_REGISTER_WORKLOAD(igemm_f32_out_f32_acc);

    using sgemm_f32_acc = CublasGemmEx<Shapes::TypeConfig<float, float, float>, int>;
    BASELINER_REGISTER_WORKLOAD(sgemm_f32_acc);

    // TF32 Fast path (Common for Ampere+ GPUs)
    using sgemm_f32_fast_tf32 = CublasGemmEx<Shapes::TypeConfig<float, float, float, FastTF32Math>, int>;
    BASELINER_REGISTER_WORKLOAD(sgemm_f32_fast_tf32);

    // Fast 16F path (Down-converts internally)
    using sgemm_f32_fast_16f = CublasGemmEx<Shapes::TypeConfig<float, float, float, Fast16FMath>, int>;
    BASELINER_REGISTER_WORKLOAD(sgemm_f32_fast_16f);

    // --- Double Precision (Compute 64F) ---
    using dgemm_f64_acc = CublasGemmEx<Shapes::TypeConfig<double, double, double>, int>;
    using dgemm_f64_acc_pedantic = CublasGemmEx<Shapes::TypeConfig<double, double, double, PedanticMath>, int>;
    ...
    using cgemm_c32_acc = CublasGemmEx<Shapes::TypeConfig<cuComplex, float, cuComplex>, int>;
    BASELINER_REGISTER_WORKLOAD(cgemm_c32_acc);

    // --- Complex Double Precision (Compute 64F) ---
    using zgemm_c64_acc = CublasGemmEx<Shapes::TypeConfig<cuDoubleComplex, double, cuDoubleComplex>, int>;
